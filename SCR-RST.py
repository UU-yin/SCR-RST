...
def validate_two_column_data(two_column_input):
    """验证两列数据输入"""
    try:
...
def analyze_two_column_data():
    ...
    try:
        ...
        label_data_pairs, valid_pairs, invalid_lines, decimal_info = \
            validate_two_column_data(two_column_input)
...
def iterative_robust_algorithm(data, max_iterations=50, k=1.5, scheme="strict"):
    ...
    if scheme == "presentation":
        ...
        Z_scores_rounded = np.round((data - formatted_X_star) / formatted_S_star, 2)
        ...
        Z_scores_rounded = np.round((data - X_star) / S_star, 2)
...
def quartile_robust_algorithm(data, scheme="strict"):
    ...
    if scheme == "presentation":
        ...
        Z_scores_rounded = np.round((data - formatted_median) / formatted_niqr, 2)
        ...
        Z_scores_rounded = np.round((data - median) / niqr, 2)
...
def z_score_calculation_algorithm(data, robust_mean, robust_std, scheme="strict"):
    ...
    try:
        ...
        if scheme == "presentation":
            ...
            Z_scores_rounded = np.round((data_array - formatted_robust_mean) / formatted_robust_std, 2)
            ...
            Z_scores_rounded = np.round((data_array - robust_mean_val) / robust_std_val, 2)
...
def q_hampel_robust_algorithm(data, scheme="strict"):
    ...
    try:
        ...
        if robust_std > 1e-12:
            Z_scores_rounded = np.round((data_array - robust_mean) / robust_std, 2).tolist()
        else:
...
def _format_q_hampel_results(data, robust_mean, robust_std, clean_data, outliers, 
    ...
    if scheme == "presentation":
        ...
        if formatted_robust_std > 1e-12:
            Z_scores_rounded = np.round((data - formatted_robust_mean) / formatted_robust_std, 2).tolist()
        else:
...
def _fallback_method(data, scheme, error_message=""):
    ...
    if std_val > 0:
        Z_scores_rounded = np.round((data_array - mean_val) / std_val, 2).tolist()
    else:
...
def display_analysis_results(results, method, data, original_labels=None):
    """统一显示分析结果 - 整合所有显示逻辑，避免重复"""
    ...
    if z_scores_data:
        try:
            ...
            with col1:
                sat_percent = satisfactory/total*100 if total > 0 else 0
                st.metric("满意 (|Z| ≤ 2)", f"{satisfactory} 个", f"{sat_percent:.1f}%")
            with col2:
                quest_percent = questionable/total*100 if total > 0 else 0
                st.metric("可疑 (2 < |Z| < 3)", f"{questionable} 个", f"{quest_percent:.1f}%")
            with col3:
                unsat_percent = unsatisfactory/total*100 if total > 0 else 0
    ...
    # 显示Z比分对比表格
    n_points = len(results['Z_scores_rounded'])
    ...
    chart_data = pd.DataFrame({
        'Label': labels,
        'Z_Score': results['Z_scores_rounded'],
        'Classification': results['z_score_classifications']
    })
...
def display_scheme_comparison(data, method, k_value, max_iter, calculation_scheme):
    """显示计算方案对比"""
    st.markdown("---")
    st.subheader("🔍 计算方案对比")
...
def display_export_section(results, method, data, input_method, original_labels=None):
    """显示导出部分"""
    st.subheader("💾 导出结果")
    ...
    if input_method == "带编号数据输入" and st.session_state.label_data_pairs:
        ...
        for label, value in st.session_state.label_data_pairs:
            z_score = None
            if value is not None and valid_data_count < len(results['Z_scores_rounded']):
                z_score = results['Z_scores_rounded'][valid_data_count]
        ...
        if current_original_data:
            for i, value in enumerate(current_original_data):
                ...
                if value is not None:
                    z_score = results['Z_scores_rounded'][valid_data_count] if valid_data_count < len(results['Z_scores_rounded']) else None
            ...
            for i, value in enumerate(data):
                original_label = f"{str(i+1).zfill(3)}"
                z_score = results['Z_scores_rounded'][i] if i < len(results['Z_scores_rounded']) else None
    ...
    with export_col4:
        scheme_text = "严格计算方案" if results['calculation_scheme'] == "strict" else "规范展示方案"
        report = f"""                
{method}分析报告
================
        ...
        for i in range(len(result_df)):
            ...
            if pd.isna(row['输入数据']):
                ...
                if detected_decimal_places == 0:
                    ...
                    input_data = f"{row['输入_data']:.{detected_decimal_places}f}"
...
def main():
    ...
    if data is not None and len(data) > 0:
        try:
            ...
            # 使用统一的显示函数，避免重复
            comparison_df, fig = display_analysis_results(results, method, data, original_labels)
            
            # 方案比较功能
            if show_scheme_comparison and method != "Z比分计算模块":
                display_scheme_comparison(data, method, k_value, max_iter, calculation_scheme)
...
"""
        ...
        # 修复这里：使用正确的列名'输入数据'，而不是'输入_data'
        for i in range(len(result_df)):
            row = result_df.iloc[i]
            # 修复这里：正确的列名是'输入数据'
            input_data_value = row['输入数据']
            if pd.isna(input_data_value):
                input_data = ""
            else:
                if detected_decimal_places == 0:
                    input_data = f"{int(input_data_value)}"
                else:
                    input_data = f"{input_data_value:.{detected_decimal_places}f}"
